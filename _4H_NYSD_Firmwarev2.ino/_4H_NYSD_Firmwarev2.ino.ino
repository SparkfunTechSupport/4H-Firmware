...
void setup() {
  ...
  Serial.begin(115200);
  pinMode(5, OUTPUT);
  pinMode(4, INPUT_PULLUP);//pull-up for tilt Sensor
//**************** Change SSID in next line "" *********************
...
void switchPressed ()
{
  if (digitalRead (4) == LOW)//digitalRead (4) == LOW
  {
      state = 1;
      digitalWrite (5, HIGH);
    if (digitalRead (4) == LOW&&(millis()-last_interrupt>400)) //digitalRead (4) == LOW&&(millis()-last_interrupt>400)
    {                                                          
      counter++;
      state = 0;
